...
def test_extract_tool_call_accepts_simple_hermes_shape():
    text = (
        '<tool_call>{"name":"deepseen_smart_image_recreations_create_and_wait",'
        '"arguments":{"keywords":"tee","product_images":["https://example.com/a.png"]}}</tool_call>'
    )

    tool_calls, cleaned = _extract_tool_calls_from_text(text)

    assert len(tool_calls) == 1
    assert tool_calls[0].function.name == "deepseen_smart_image_recreations_create_and_wait"
    assert json.loads(tool_calls[0].function.arguments) == {
        "keywords": "tee",
        "product_images": ["https://example.com/a.png"],
    }
    assert cleaned == ""


def test_extract_tool_call_keeps_non_tool_text():
    text = (
        '先执行工具\n'
        '<tool_call>{"name":"deepseen_smart_image_recreations_create_and_wait","arguments":{"keywords":"tee"}}</tool_call>\n'
        '然后告诉我结果'
    )
    ...
    assert len(tool_calls) == 1
    assert tool_calls[0].function.name == "deepseen_smart_image_recreations_create_and_wait"
    assert cleaned == "先执行工具\n然后告诉我结果"


def test_prompt_warns_against_terminal_fallback_for_deepseen_tools():
    prompt = _format_messages_as_prompt(
        [{"role": "user", "content": "给我做一张商品图"}],
        tools=[
            {
                "function": {
    ...
    assert "Tool-call JSON MUST use plain ASCII double quotes only." in prompt
    assert "Never claim that a tool is running" in prompt
    assert "Deepseen rule:" in prompt
    assert "do NOT fall back to terminal/process/execute_code" in prompt
