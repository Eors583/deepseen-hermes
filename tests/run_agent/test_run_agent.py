...
class TestBuildAssistantMessage:
    ...
    def test_recovers_code_fenced_tool_call_from_content(self, agent):
        msg = _mock_assistant_msg(
            content=(
                "```json\n"
                "{\n"
                ...
                '    "arguments": {\n'
                '      "keywords": "men cotton t-shirt streetwear graphic tee",\n'
                '      "product_images": ["https://example.com/p.png"]\n'
                "    }\n"
                "  }\n"
                "}\n"
                "```"
        ...
        result = agent._build_assistant_message(msg, "stop")
        assert result["tool_calls"][0]["function"]["name"] == "deepseen_smart_image_recreations_create_and_wait"
    ...
    def test_recovers_xml_tool_call_from_content_and_keeps_prose(self, agent):
        msg = _mock_assistant_msg(
            content=(
                "先调用工具。\n"
                "<tool_call>\n"
                '{"name":"deepseen_smart_image_recreations_create_and_wait","arguments":{"keywords":"tee","product_images":["https://example.com/p.png"]}}'
                "\n</tool_call>\n"
                "稍后返回结果。"
            ),
            tool_calls=None,
