...
async function uploadFiles(attachments: Attachment[]): Promise<UploadedAttachment[]> {
  ...
  const data = await res.json() as { files: UploadedAttachment[] }
  return data.files
}
    ...
    for (let i = 0; i < uploadedFiles.length; i++) {
      ...
      if (attachment?.type.startsWith('image/')) {
        blocks.push({
          ...
          resource_file_id: uploaded.resource_file_id,
          resource_purpose: uploaded.resource_purpose,
          resource_content_type: uploaded.resource_content_type,
        })
      } else {
        // Other files
        blocks.push({
  ...
  for (const msg of filteredMsgs) {
    ...
    if (msg.role === 'tool') {
      ...
      result.push({
        ...
        toolStatus: runtimeToolOutputHasError((msg as any).content) ? 'error' : 'done',
        finishReason: readFinishReason(msg),
        runMarker: readRunMarker(msg),
      })
      continue
...
const DEEPSEEN_MEDIA_TOOLS = new Set([
  ...
  'deepseen_smart_image',
  'deepseen_smart_video',
  'deepseen_image_recreation',
  'deepseen_video_recreation',
])

function isDeepSeenMediaTool(name?: string): boolean {
  const value = String(name || '')
  return DEEPSEEN_MEDIA_TOOLS.has(value) || /deepseen_.*(image|video).*(create|recreation|smart)/.test(value)
}
...
export const useChatStore = defineStore('chat', () => {
  ...
  const activeMediaGenerationProgress = computed(() => {
    const items = messages.value.filter(
      message => message.role === 'tool'
        && isDeepSeenMediaTool(message.toolName)
        && (message.toolProgress || message.toolStatus === 'running'),
    )
  ...
  function handleToolProgress(sessionId: string, evt: RunEvent) {
    const msgs = getSessionMsgs(sessionId)
    const toolName = String((evt as any).tool || (evt as any).name || (evt as any).tool_name || '')
    if (!isDeepSeenMediaTool(toolName)) return
    const toolCallId = (evt as any).tool_call_id as string | undefined
    const candidates = toolCallId
      ? msgs.filter(m => m.role === 'tool' && m.toolCallId === toolCallId)
      : msgs.filter(m => m.role === 'tool' && isDeepSeenMediaTool(m.toolName))
    const target = candidates[candidates.length - 1]
    if (!target) return
    const rawProgress = (evt as any).progress
    const progress = typeof rawProgress === 'number' && Number.isFinite(rawProgress)
      ? rawProgress
      : null
    const previous = target.toolProgress
    const nextEvent = {
      timestamp: Date.now(),
      status: (evt as any).status ? String((evt as any).status) : undefined,
      phase: (evt as any).phase ? String((evt as any).phase) : undefined,
      stage: (evt as any).stage ? String((evt as any).stage) : undefined,
      message: String((evt as any).message || evt.preview || evt.text || '').trim() || undefined,
      progress,
    }
    updateMessage(sessionId, target.id, {
      toolName: target.toolName || toolName,
      toolPreview: nextEvent.message || target.toolPreview,
      toolProgress: {
        status: nextEvent.status || previous?.status,
        phase: nextEvent.phase || previous?.phase,
        stage: nextEvent.stage || previous?.stage,
        message: nextEvent.message || previous?.message,
        progress: progress ?? previous?.progress ?? null,
        jobId: (evt as any).job_id ? String((evt as any).job_id) : previous?.jobId,
        action: (evt as any).action ? String((evt as any).action) : previous?.action,
        outputUrls: Array.isArray((evt as any).output_urls)
          ? (evt as any).output_urls.map(String)
          : previous?.outputUrls,
        updatedAt: Date.now(),
        events: [...(previous?.events || []), nextEvent].slice(-50),
        raw: (evt as any).raw ?? previous?.raw,
      },
    })
  }
  ...
  async function sendMessage(content: string, attachments?: Attachment[]) {
    ...
    try {
      ...
      if (attachments && attachments.length > 0) {
        ...
        const uploadMap = new Map(uploaded.map(f => [f.name, f]))
        if (shouldQueue && userMsg.attachments) {
          userMsg.attachments = userMsg.attachments.map(a => {
            const uploadedAttachment = uploadMap.get(a.name)
            if (!uploadedAttachment) return a
            return {
              ...a,
              url: uploadedAttachment.resource_url || getDownloadUrl(uploadedAttachment.path, uploadedAttachment.name),
              resource_url: uploadedAttachment.resource_url,
              resource_file_id: uploadedAttachment.resource_file_id,
            }
          })
