...
function looksLikeVideoUrl(path: string): boolean {
...
function looksLikeAudioUrl(path: string): boolean {
  return hasExtension(path, AUDIO_EXTENSIONS)
}

function mediaSrc(path: string): string {
  return isLocalFilePath(path) ? getDownloadUrl(normalizeLocalFilePath(path)) : path
}

function renderMediaLink(rawPath: string, filename: string): string | null {
  const decodedPath = decodeHtmlAttr(rawPath)
  if (!isLocalFilePath(decodedPath) && !isHttpUrl(decodedPath)) return null

  const fileName = filename.trim()
  const src = escapeHtmlAttr(mediaSrc(decodedPath))
  const safeName = escapeHtmlAttr(fileName)

  if (looksLikeVideoUrl(decodedPath)) {
    return `<div class="markdown-video-container">
      <video class="markdown-video" controls preload="metadata" src="${src}"></video>
  ...
  if (looksLikeAudioUrl(decodedPath)) {
    return `<div class="markdown-audio-container">
      <audio class="markdown-audio" controls preload="metadata" src="${src}"></audio>
...
const renderedHtml = computed(() => {
  ...
  // Replace file/media links with file card UI or video/audio player.
  ...
  html = html.replace(/<a href="([^"]+)">([^<]+)<\/a>/g, (match, rawPath, filename) => {
    const mediaHtml = renderMediaLink(rawPath, filename)
