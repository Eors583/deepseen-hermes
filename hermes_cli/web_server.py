...
class DeepSeenKeyUpdate(BaseModel):
    api_key: str
    profile: Optional[str] = None
...
def _chat_upload_profile(request: Request) -> str:
    raw = (request.headers.get("X-Hermes-Profile") or "").strip() or "default"
    if "/" in raw or "\\" in raw or ".." in raw:
        raise HTTPException(status_code=400, detail="Invalid profile name")
    return raw or "default"


def _chat_upload_dir(profile: str) -> Path:
    root = get_hermes_home() / "upload" / profile
    root.mkdir(parents=True, exist_ok=True)
    return root.resolve()


def _split_multipart(raw: bytes, boundary: bytes) -> List[bytes]:
    parts: List[bytes] = []
    start = 0
    while True:
        idx = raw.find(boundary, start)
        if idx == -1:
            break
        if start > 0:
            part = raw[start + 2:idx]
            if part.strip(b"\r\n-"):
                parts.append(part)
...
def _multipart_filename(header: str) -> str:
...
def _guess_upload_purpose(content_type: str, filename: str) -> str:
...
def _upload_to_deepseen_resource(path: Path, purpose: str) -> Dict[str, Any]:
    plugin_path = PROJECT_ROOT / "plugins" / "crossborder-deepseen" / "__init__.py"
    if not plugin_path.is_file():
        raise RuntimeError("DeepSeen plugin is not installed")
    spec = importlib.util.spec_from_file_location("crossborder_deepseen_upload", plugin_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("DeepSeen plugin could not be loaded")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    raw = module._call_runner("upload_file", {"local_path": str(path), "purpose": purpose})
    parsed = json.loads(raw)
    if not isinstance(parsed, dict) or not parsed.get("ok"):
        error = parsed.get("error") if isinstance(parsed, dict) else None
        if isinstance(error, dict):
            raise RuntimeError(str(error.get("message") or error.get("code") or "DeepSeen upload failed"))
        raise RuntimeError("DeepSeen upload failed")
    return parsed
...
async def upload_chat_attachments(request: Request):
    ...
    boundary_match = re.search(r"boundary=(?P<boundary>[^;]+)", content_type, re.I)
    if not boundary_match:
        raise HTTPException(status_code=400, detail="Missing boundary")
    boundary_text = boundary_match.group("boundary").strip().strip('"')
    ...
    async for chunk in request.stream():
        ...
        if total > _CHAT_UPLOAD_MAX_BYTES:
            raise HTTPException(
                status_code=413,
                detail=f"File too large (max {_CHAT_UPLOAD_MAX_BYTES // (1024 * 1024)}MB)",
            )
        chunks.append(chunk)

    profile = _chat_upload_profile(request)
    upload_dir = _chat_upload_dir(profile)
    raw = b"".join(chunks)
    files: List[Dict[str, Any]] = []

    for part in _split_multipart(raw, boundary):
        header_end = part.find(b"\r\n\r\n")
        if header_end == -1:
            continue
        header_bytes = part[:header_end]
        header = header_bytes.decode("utf-8", errors="replace")
        filename = Path(_multipart_filename(header)).name
        if not filename:
            continue
        ...
        media_type = (content_type_match.group(1).strip() if content_type_match else "") or (
            mimetypes.guess_type(filename)[0] or "application/octet-stream"
        )
        purpose = _guess_upload_purpose(media_type, filename)
        entry: Dict[str, Any] = {
            "name": filename,
            "path": str(saved_path),
            "resource_purpose": purpose,
            "resource_content_type": media_type,
        }
        try:
            uploaded = _upload_to_deepseen_resource(saved_path, purpose)
            file_info = uploaded.get("file") if isinstance(uploaded.get("file"), dict) else uploaded
            if isinstance(file_info, dict):
                entry["resource_file_id"] = file_info.get("id") or uploaded.get("id")
                entry["resource_url"] = file_info.get("url") or uploaded.get("url")
                entry["resource_content_type"] = (
                    file_info.get("content_type")
                    or uploaded.get("content_type")
                    or entry["resource_content_type"]
                )
                entry["resource_purpose"] = file_info.get("purpose") or uploaded.get("purpose") or purpose
        except Exception as exc:
            entry["resource_error"] = str(exc)
        files.append(entry)
...
class HermesWebUiBatchDeleteSessionTarget(BaseModel):
    id: str
    profile: Optional[str] = None


class HermesWebUiBatchDeleteSessions(BaseModel):
    ids: List[str] = []
    sessions: Optional[List[HermesWebUiBatchDeleteSessionTarget]] = None
...
def _session_row_for_webui(row: Dict[str, Any], profile: Optional[str] = None) -> Dict[str, Any]:
    out = dict(row)
    out.setdefault("source", "tui")
    out.setdefault("model", "")
    out.setdefault("provider", out.get("billing_provider") or "")
    out.setdefault("title", None)
    out.setdefault("tool_call_count", 0)
    out.setdefault("input_tokens", 0)
    out.setdefault("output_tokens", 0)
    out.setdefault("cache_read_tokens", 0)
    out.setdefault("cache_write_tokens", 0)
    out.setdefault("reasoning_tokens", 0)
    out.setdefault("billing_provider", out.get("provider") or None)
    out.setdefault("estimated_cost_usd", 0)
    out.setdefault("actual_cost_usd", None)
    out.setdefault("cost_status", "unknown")
    out["profile"] = profile or "herbound"
    out["webui_imported"] = True
    return out


@app.post("/api/hermes/sessions/batch-delete")
async def batch_delete_hermes_webui_sessions(
    body: HermesWebUiBatchDeleteSessions,
    request: Request,
):
    """Delete sessions from the Hermes Web UI compatibility surface.
    ...
    process default.
    """
    targets = body.sessions or [
        HermesWebUiBatchDeleteSessionTarget(id=session_id)
    ...
    for target in targets:
        session_id = (target.id or "").strip()
        if not session_id:
            continue
        db_profile = _hermes_webui_profile(target.profile, request)
        grouped.setdefault(db_profile, []).append(session_id)

    deleted = 0
    errors: List[Dict[str, str]] = []
    for db_profile, session_ids in grouped.items():
        db = _open_session_db_for_profile(db_profile)
        try:
            for session_id in session_ids:
                sid = db.resolve_session_id(session_id)
                if not sid:
                    errors.append({"id": session_id, "error": "Session not found"})
                    continue
...
async def get_hermes_webui_sessions(
    request: Request,
    ...
    db_profile = _hermes_webui_profile(profile, request)
    db = _open_session_db_for_profile(db_profile)
    try:
        sessions = db.list_sessions_rich(
            source=source or None,
        ...
        return {
            "sessions": [_session_row_for_webui(s, profile or "herbound") for s in sessions],
            "total": db.session_count(source=source or None, include_archived=False, exclude_children=True),
            "limit": limit,
            "offset": offset,
        }
    finally:
...
@app.get("/api/hermes/sessions/hermes")
async def get_hermes_webui_native_sessions(
    request: Request,
    limit: int = 50,
    offset: int = 0,
    profile: Optional[str] = None,
):
    return await get_hermes_webui_sessions(
        request=request,
        source=None,
        limit=limit,
        offset=offset,
...
async def get_hermes_webui_session_detail(
    session_id: str,
    request: Request,
    profile: Optional[str] = None,
):
    ...
    try:
        ...
        detail = _session_row_for_webui(session, profile or "herbound")
...
async def delete_hermes_webui_session(
    session_id: str,
    request: Request,
    profile: Optional[str] = None,
):
...
@app.get("/api/hermes/sessions/hermes/{session_id}")
async def get_hermes_webui_native_session_detail(
    session_id: str,
    request: Request,
    profile: Optional[str] = None,
):
    return await get_hermes_webui_session_detail(session_id, request, profile)


@app.get("/api/hermes/sessions/conversations/{session_id}/messages/paginated")
async def get_hermes_webui_session_messages_page(
    session_id: str,
    request: Request,
    offset: int = 0,
    limit: int = 300,
    ...
    try:
        ...
        return {
            "session": _session_row_for_webui(session, profile or "herbound"),
            "messages": page,
            "total": total,
            "offset": safe_offset,
            "limit": safe_limit,
...
class CachedStaticFiles(StaticFiles):
    """Serve hashed Vite assets with browser cache headers."""
...
def mount_spa(application: FastAPI):
    ...
    application.mount("/assets", CachedStaticFiles(directory=WEB_DIST / "assets"), name="assets")
