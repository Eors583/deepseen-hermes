...
const TOOL_CREDENTIALS = [
  {
    key: 'DEEPSEEN_API_KEY',
    label: 'Deepseen API Key',
    description: 'Required for Deepseen ecommerce image/video creative tools.',
    required: true,
    secret: true,
  },
  {
    key: 'DEEPSEEN_BASE_URL',
    label: 'Deepseen Base URL',
    description: 'Optional Deepseen Open API base URL override.',
    required: false,
    secret: false,
    placeholder: 'https://deepseen.ai/v1',
  },
] as const
...
async function readProfileEnv(profile: string): Promise<Record<string, string>> {
  try {
    return parseEnv(await readFile(envPath(profile), 'utf-8'))
  } catch {
    return {}
  }
}
...
export async function getToolCredentials(ctx: any) {
  try {
    const profile = requestedProfile(ctx)
    const env = await readProfileEnv(profile)
    ctx.body = {
      credentials: TOOL_CREDENTIALS.map((item) => {
        const value = env[item.key] || ''
        return {
          ...item,
          is_set: Boolean(value),
          redacted_value: item.secret ? redactSecret(value) : value,
        }
      }),
    }
  } catch (err: any) {
...
export async function updateToolCredentials(ctx: any) {
  const { values, restart } = ctx.request.body as { values?: Record<string, string>; restart?: boolean }
  if (!values || typeof values !== 'object') {
    ctx.status = 400
    ctx.body = { error: 'Missing values' }
    return
  }

  const allowed = new Set<string>(TOOL_CREDENTIALS.map((item) => item.key))
  const entries = Object.entries(values).filter(([key]) => allowed.has(key))
  if (entries.length === 0) {
    ctx.status = 400
    ctx.body = { error: 'No supported tool credentials provided' }
    return
  }
  ...
  try {
    ...
    for (const [key, value] of entries) {
      await saveEnvValueForProfile(profile, key, String(value || '').trim())
    }

    if (restart !== false) {
      try {
        const restartResult = await restartGatewayForProfile(profile)
        logger.info('[config] gateway restarted after tool credential update profile=%s result=%j', profile, restartResult)
      } catch (err) {
        logger.error(err, 'Gateway restart failed')
        ctx.status = 500
        ctx.body = { error: err instanceof Error ? err.message : 'Gateway restart failed' }
