...
import { uploadToDeepseenResource } from '../services/hermes/deepseen-openapi'
...
export async function handleUpload(ctx: any) {
  ...
  const uploadDir = getProfileUploadDir(profile)
  ...
  for (const part of parts) {
    ...
    const purpose = contentType.startsWith('video/') ? 'reference_video' : 'product_image'
    try {
      const uploadedResource = await uploadToDeepseenResource({
        profile,
        buffer: data,
        filename,
        contentType,
      ...
      results.push({
        name: filename,
        path: savedPath,
        resource_file_id: uploadedResource?.id,
        resource_url: uploadedResource?.url,
        resource_purpose: purpose,
        resource_content_type: contentType,
      })
    } catch (err: any) {
      results.push({
        name: filename,
        ...
        resource_purpose: purpose,
        resource_content_type: contentType,
        resource_error: err instanceof Error ? err.message : String(err),
      })
    }
  }
