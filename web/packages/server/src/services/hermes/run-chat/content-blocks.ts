...
function preferredRemoteUrl(block: Extract<ContentBlock, { type: 'image' | 'file' }>): string | null {
  return typeof block.resource_url === 'string' && block.resource_url.trim()
    ? block.resource_url.trim()
    : null
}

function resourceHint(block: Extract<ContentBlock, { type: 'image' | 'file' }>): string {
  const parts: string[] = []
  if (block.resource_url) parts.push(`Uploaded resource URL: ${block.resource_url}`)
  if (block.resource_file_id) parts.push(`Deepseen file_id: ${block.resource_file_id}`)
  return parts.length ? `\n${parts.join('\n')}` : ''
}
...
export async function convertContentBlocks(blocks: ContentBlock[]): Promise<ResponseContentPart[]> {
  ...
  for (const block of blocks) {
    if (block.type === 'text') {
      ...
      const remoteUrl = preferredRemoteUrl(block)
      if (remoteUrl) {
        parts.push({ type: 'input_image', image_url: remoteUrl })
        continue
      }
  ...
  for (const block of blocks) {
    if (block.type === 'text') {
      ...
      parts.push({
        type: 'text',
        text: remoteUrl
          ? `[Attached image: ${block.name || block.path}]\nPrimary remote resource URL for tools: ${remoteUrl}${resourceHint(block)}`
          : `[Attached image: ${block.name || block.path}]\nLocal image path for tools: ${block.path}${resourceHint(block)}`,
      })
      if (remoteUrl) {
        parts.push({ type: 'image_url', image_url: { url: remoteUrl } })
        continue
      }
      const dataUri = await imageBlockToDataUri(block)
      if (dataUri) {
      ...
      parts.push({
        type: 'text',
        text: remoteUrl
          ? `[Attached file: ${block.name || block.path}]\nPrimary remote resource URL for tools: ${remoteUrl}${resourceHint(block)}`
          : `[Attached file: ${block.name || block.path}]\nLocal file path for tools: ${block.path}${resourceHint(block)}`,
      })
    }
  }
  return parts
