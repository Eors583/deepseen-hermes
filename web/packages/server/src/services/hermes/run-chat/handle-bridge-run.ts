...
import {
  generatedMediaMarkdown,
  mediaGenerationStartedPreview,
...
export async function handleBridgeRun(
  ...
  state.bridgeEmittedMediaKeys = new Set()
  ...
  for (const ev of chunk.events || []) {
    ...
    if (evType === 'session.title.updated') {
      ...
      const payload = {
        ...
        preview: mediaGenerationStartedPreview(toolName) || ev.preview || summarizeToolArguments(tool.arguments),
      }
      ...
      const mediaMarkdown = generatedMediaMarkdown(toolName, completed.output)
      if (mediaMarkdown && !state.bridgeEmittedMediaKeys?.has(mediaMarkdown)) {
        state.bridgeEmittedMediaKeys = state.bridgeEmittedMediaKeys || new Set()
        state.bridgeEmittedMediaKeys.add(mediaMarkdown)
        processBridgeTextDelta(state, sessionId, runMarker, chunk.run_id, mediaMarkdown, emit)
      }
    } else if (evType === 'tool.progress') {
      const toolName = (ev.tool_name as string) || ''
      const payload = {
        event: 'tool.progress',
        run_id: chunk.run_id,
        tool_call_id: typeof ev.tool_call_id === 'string' ? ev.tool_call_id : '',
        tool: toolName,
        name: toolName,
        arguments: ev.args,
        preview: ev.text || ev.preview || '',
      ...
      pushState(sessionMap, sessionId, 'tool.progress', payload)
      emit('tool.progress', payload)
