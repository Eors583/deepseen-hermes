...
export class AgentBridgeManager {
  ...
  private async startProcess(): Promise<void> {
    ...
    logger.info(
      '[agent-bridge] starting: command=%s args=%s cwd=%s agentRoot=%s hermesHome=%s endpoint=%s',
      command.command,
      args.join(' '),
      process.cwd(),
      agentRoot ?? '<unset>',
    ...
    child.once('exit', (code, signal) => {
      ...
      logger.warn(
        '[agent-bridge] exited code=%s signal=%s stdoutTail=%s stderrTail=%s',
        code,
        signal,
        stdoutTail || '<empty>',
        stderrTail || '<empty>',
    ...
    await new Promise<void>((resolveReady, rejectReady) => {
      ...
      const onExitBeforeReady = (code: number | null, signal: NodeJS.Signals | null) => {
        cleanup()
        logger.warn(
          '[agent-bridge] exited before ready code=%s signal=%s stdoutTail=%s stderrTail=%s',
          code,
          signal,
          stdoutTail || '<empty>',
          stderrTail || '<empty>',
