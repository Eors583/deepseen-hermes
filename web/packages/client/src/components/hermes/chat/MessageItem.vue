...
const toolExpandedTouched = ref(false);
...
const thinkingExpanded = computed(() => {
  ...
  return settingsStore.display.show_reasoning !== false;
});
...
watch(
  () => [props.message.role, props.message.toolStatus, props.message.toolName, hasToolDetails.value] as const,
  ([role, status, toolName, details]) => {
    if (toolExpandedTouched.value) return;
    if (role !== 'tool' || !details) return;
    if (status === 'running' || String(toolName || '').startsWith('deepseen_')) {
  ...
  >
    <template v-if="message.role === 'tool'">
      <div
        ...
        @click="if (hasToolDetails) { toolExpandedTouched = true; toolExpanded = !toolExpanded; }"
      >
        <svg
          v-if="hasToolDetails"
          width="10"
