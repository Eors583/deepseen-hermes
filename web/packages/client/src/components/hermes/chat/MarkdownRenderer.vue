...
function pathExtension(path: string): string {
  try {
    const parsed = isRemoteUrl(path) ? new URL(path) : null
    const cleanPath = parsed ? parsed.pathname : path.split(/[?#]/, 1)[0]
...
function isVideoPath(path: string, filename = ''): boolean {
  const ext = pathExtension(path) || pathExtension(filename)
  return ext === 'mp4' || ext === 'webm' || ext === 'mov'
}

function renderVideoCard(rawPath: string, filename: string): string {
  const isLocal = isLocalFilePath(rawPath)
  const path = isLocal ? normalizeLocalFilePath(rawPath) : rawPath
  const src = isLocal ? getDownloadUrl(path) : path
  const safeSrc = escapeHtml(src)
  const safeName = escapeHtml(filename || path.split('/').pop() || 'video')
  const downloadControl = isLocal
    ? `<button class="markdown-video-download" type="button" data-path="${escapeHtml(path)}" data-filename="${safeName}" title="${t('download.downloadFile')}" aria-label="${t('download.downloadFile')}">`
      ...
      + `</svg></button>`
    : `<a class="markdown-video-download" href="${safeSrc}" target="_blank" rel="noopener noreferrer" download title="${t('download.downloadFile')}" aria-label="${t('download.downloadFile')}">`
      + `<svg class="att-download-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">`
      + `<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />`
      + `<polyline points="7 10 12 15 17 10" />`
      + `<line x1="12" y1="15" x2="12" y2="3" />`
  ...
  return `<div class="markdown-video-container">
    <video class="markdown-video" controls preload="metadata" src="${safeSrc}"></video>
...
const renderedHtml = computed(() => {
  ...
  // Video URLs from generation tools may be remote signed URLs, so those are
  // also rendered as playable media when the path/label has a video extension.
  html = html.replace(/<a href="([^"]+)">([^<]+)<\/a>/g, (match, rawPath, filename) => {
    const fileName = filename.trim()

    if (!isLocalFilePath(rawPath)) {
      return isRemoteUrl(rawPath) && isVideoPath(rawPath, fileName)
        ? renderVideoCard(rawPath, fileName)
        : match
    }
    ...
    if (isVideoPath(path, fileName)) {
      return renderVideoCard(path, fileName)
    }

    // Other files: render as file card
    return `<div class="markdown-file-card" data-path="${escapeHtml(path)}" data-filename="${escapeHtml(fileName)}" title="${t('download.downloadFile')}">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
...
async function handleMarkdownClick(event: MouseEvent): Promise<void> {
  ...
  const videoDownload = target.closest('.markdown-video-download') as HTMLElement | null
  if (videoDownload) {
    const path = videoDownload.getAttribute('data-path')
    if (path) {
      event.preventDefault()
      event.stopPropagation()
      const fileName = videoDownload.getAttribute('data-filename') || undefined
