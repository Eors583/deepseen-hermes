...
async function uploadGroupFiles(attachments: Attachment[]): Promise<UploadedAttachment[]> {
...
function buildGroupContentBlocks(content: string, attachments: Attachment[], files: UploadedAttachment[]): ContentBlock[] {
    ...
    for (let i = 0; i < files.length; i += 1) {
        ...
        if (attachment?.type.startsWith('image/')) {
            blocks.push({
                ...
                resource_file_id: file.resource_file_id,
                resource_purpose: file.resource_purpose,
                resource_content_type: file.resource_content_type,
            })
        } else {
            blocks.push({
                type: 'file',
