...
export const useChatStore = defineStore('chat', () => {
          ...
          if (data.events?.length) {
            for (const evt of data.events) {
              ...
              if (e.event === 'compression.started') {
                ...
                const hasError = e.error === true || runtimeToolOutputHasError(output)
                updateMessage(sessionId, last.id, {
                  toolStatus: hasError ? 'error' : 'done',
                  toolDuration: e.duration,
                  toolResult: output,
                  toolProgress: finalizeToolProgressState(
                    last.toolProgress,
                    hasError ? 'error' : 'completed',
                    hasError ? 'Deepseen SDK 生成失败' : 'Deepseen SDK 已完成',
                  ),
                })
                }
              } else if (e.event === 'tool.progress') {
                handleToolProgressEvent(sessionId, e as RunEvent)
  ...
  function buildToolProgressState(previous: ToolProgressState | undefined, evt: RunEvent): ToolProgressState {
    const timestamp = normalizeProgressTimestamp(evt.timestamp)
    const entry: ToolProgressEntry = {
      stage: typeof (evt as any).stage === 'string' ? (evt as any).stage : undefined,
      status: typeof (evt as any).status === 'string' ? (evt as any).status : undefined,
      progress: typeof (evt as any).progress === 'number' ? Math.max(0, Math.min(100, (evt as any).progress)) : undefined,
      text: String(evt.text || evt.preview || 'Deepseen SDK 正在处理中'),
      timestamp,
      detail: (evt as any).detail,
    }
    const previousEvents = Array.isArray(previous?.events) ? previous.events : []
    const events = [...previousEvents, entry].slice(-60)
    return {
      stage: entry.stage,
      status: entry.status,
  ...
  function finalizeToolProgressState(
    previous: ToolProgressState | undefined,
    ...
    const entry: ToolProgressEntry = {
      ...
      text,
      timestamp,
      detail: previous.events[previous.events.length - 1]?.detail,
    }
    return {
      stage: previous.stage,
      status,
      ...
      text,
      updatedAt: timestamp,
      events: [...previous.events, entry].slice(-60),
    }
  }

  function handleToolProgressEvent(sessionId: string, evt: RunEvent) {
    const msgs = getSessionMsgs(sessionId)
    const toolCallId = (evt as any).tool_call_id as string | undefined
    const toolName = String(evt.tool || evt.name || '')
    const existing = toolCallId
      ? msgs.find(m => m.role === 'tool' && m.toolCallId === toolCallId)
      : [...msgs].reverse().find(m => m.role === 'tool' && m.toolStatus === 'running' && (!toolName || m.toolName === toolName))

    if (existing) {
      updateMessage(sessionId, existing.id, {
        toolName: toolName || existing.toolName,
        toolArgs: hasRuntimeToolPayload((evt as any).arguments) ? (evt as any).arguments : existing.toolArgs,
        toolPreview: String(evt.text || evt.preview || existing.toolPreview || '').slice(0, 220) || existing.toolPreview,
        toolStatus: existing.toolStatus || 'running',
        toolProgress: buildToolProgressState(existing.toolProgress, evt),
      })
      return
    }
    ...
    addMessage(sessionId, {
      ...
      toolName,
      toolCallId,
      toolPreview: String(evt.text || evt.preview || '').slice(0, 220),
      toolArgs: runtimeToolPayloadOrUndefined((evt as any).arguments),
      toolStatus: 'running',
      toolProgress: buildToolProgressState(undefined, evt),
    })
  }
  ...
  async function sendMessage(content: string, attachments?: Attachment[]) {
    ...
    try {
      ...
      if (attachments && attachments.length > 0) {
        ...
        const uploadMap = new Map(uploaded.map(f => {
          return [f.name, f]
        }))
      ...
      const ctrl = startRunViaSocket(
        ...
        (evt: RunEvent) => {
          switch (evt.event) {
            ...
            case 'tool.progress': {
              runHadToolActivity = true
              handleToolProgressEvent(sid, evt)
              break
            }
  ...
  function resumeServerWorkingRun(sid: string, force = false) {
    ...
    registerSessionHandlers(sid, {
      ...
      onToolProgress: (evt) => handleEvent(evt),
