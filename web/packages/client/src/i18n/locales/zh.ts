export default {
  ...
  chat: {
    ...
    openHistory: '打开历史',
    hermesHistory: 'Herbound 历史',
    historyScopeHint: '这里按来源只读查看当前 profile 的 Herbound 历史会话。',
    noSessions: '暂无会话',
    searchTitle: '搜索会话',
    searchSubtitle: '按标题或消息内容搜索',
    searchScope: '搜索范围：仅 Web UI 本地会话库；不包含只读 Herbound 历史会话。',
  ...
  plugins: {
    title: '插件',
    refresh: '刷新',
    notice: '只读展示可发现的 Herbound 插件 manifest。发现元数据读取不会加载插件代码。v1 管理动作仍保留在 CLI，新 Herbound 会话生效。',
    loadFailed: '加载插件失败',
    commandCopied: '命令已复制',
  ...
  models: {
    ...
    copilotAddDetected: '已在本机检测到 GitHub Copilot OAuth 凭证，点击「添加」即可在 Herbound 中启用 Copilot。',
    ...
    copilotDeleteHintGhCli: 'Copilot 将从 Herbound 列表移除。不会影响 gh CLI —— `gh auth status` 仍显示已登录。',
    copilotDeleteHintAppsJson: 'Copilot 将从 Herbound 列表移除。不会影响 VS Code Copilot 插件的登录。',
    ...
    aliasHint: '仅修改 Web UI 显示名，发送给 Herbound 的仍是原始模型 ID。',
    ...
    visibilityHint: '仅影响 Web UI 的模型选择器和模型页展示，不会改写 Herbound CLI 的 provider/model 配置。实际调用仍使用原始模型 ID。',
  ...
  profiles: {
    ...
    rename: '重命名',
    delete: '删除',
    switchTo: '切换 Herbound Profile',
    switchConfirm: '将执行 `hermes profile use {name}` 并切换 Herbound CLI 的 active profile，是否继续？',
    switchSuccess: 'Herbound active profile 已切换为 "{name}"',
    switchFailed: '切换 Herbound Profile 失败，网关可能需要手动重启',
  ...
  codingAgents: {
    ...
    codexDescription: "OpenAI CLI，以及 Herbound openai-codex provider 的仓库任务流程。",
    copyCommand: "复制",
    commandCopied: "命令已复制",
    ...
    notes: {
      ...
      codexAuth: "添加 Herbound 管理的 OpenAI Codex OAuth 凭证。",
  ...
  skillsUsage: {
    title: '技能用量',
    subtitle: '跟踪 Herbound 会话中的技能加载和编辑',
    refresh: '刷新',
  ...
  changelog: {
    ...
    new_0_6_10_7: '官网文案统一展示为 Herbound，同时保留 npm package、CLI、仓库和环境变量等技术标识不变',
    new_0_6_10_8: 'OpenAPI 文档补充头像、辅助模型配置、Provider 模型缓存刷新和 Herbound 会话导入端点',
    ...
    new_0_6_9_1: '应用品牌统一为 Herbound，覆盖浏览器标题、侧边栏、官网标题、package metadata 和官网链接',
    ...
    new_0_6_7_5: 'Desktop 分发链路补齐 Electron 打包、应用命名、preload 构建、release artifact 上传、Windows Herbound CLI 启动、Linux 图标与可写数据路径、macOS 无证书跳过签名和 Windows 启动无窗口体验',
