export default {
  ...
  chat: {
    ...
    contextEditHint: 'Valores comunes: 256k (Herbound predeterminado), 128k (GPT-4), 32k (GPT-3.5)',
    ...
    emptyState: 'Inicia una conversacion con Herbound',
    ...
    hermesHistory: 'Historial de Herbound',
    historyScopeHint: 'Sesiones del historial de Herbound del perfil actual, de solo lectura y agrupadas por origen.',
    ...
    searchScope: 'Alcance de búsqueda: solo base de datos local de sesiones de Web UI; no incluye sesiones históricas Herbound de solo lectura.',
...
jobTriggered: 'Job ejecutado',
  ...
  plugins: {
    title: 'Plugins',
    refresh: 'Actualizar',
    notice: 'Inventario de solo lectura de manifests de plugins Herbound detectables. Los metadatos de descubrimiento se leen sin cargar código de plugins. En v1, la gestión permanece en CLI; los cambios se aplican en nuevas sesiones Herbound.',
  ...
  models: {
    ...
    copilotAddDetected: 'Se detectó un token OAuth de GitHub Copilot en este equipo. Haz clic en Agregar para habilitar Copilot en Herbound.',
    ...
    copilotDeleteHintGhCli: 'Copilot se ocultará de Herbound. Tu sesión de gh CLI no se verá afectada — `gh auth status` seguirá mostrando que estás conectado.',
    copilotDeleteHintAppsJson: 'Copilot se ocultará de Herbound. La extensión Copilot de VS Code seguirá conectada.',
    ...
    visibilityHint: 'Solo afecta al selector de modelos y a la página de modelos de Web UI. No modifica la configuración provider/model de Herbound CLI; las llamadas siguen usando el ID original del modelo.',
    ...
    aliasHint: 'Solo cambia el nombre visible en Web UI. Herbound sigue recibiendo el ID original del modelo.',
    aliasEdit: 'Renombrar',
  ...
  profiles: {
    ...
    rename: 'Renombrar',
    delete: 'Eliminar',
    switchTo: 'Cambiar Herbound Profile',
    switchConfirm: 'Esto ejecutara `hermes profile use {name}` y cambiara el active profile de Herbound CLI. Continuar?',
    switchSuccess: 'Herbound active profile cambiado a "{name}"',
    switchFailed: 'Error al cambiar Herbound Profile. Es posible que la pasarela necesite un reinicio manual.',
  ...
  codingAgents: {
    ...
    codexDescription: "CLI de OpenAI y flujo del proveedor openai-codex de Herbound para tareas de repositorio.",
    copyCommand: "Copiar",
    commandCopied: "Comando copiado",
    ...
    notes: {
      ...
      codexAuth: "Agrega credenciales OAuth de OpenAI Codex gestionadas por Herbound.",
  ...
  skillsUsage: {
    ...
    subtitle: 'Sigue las cargas y ediciones de habilidades en sesiones de Herbound',
    refresh: 'Actualizar',
  ...
  // Registro de cambios
  changelog: {
    new_0_6_10_1: 'Los textos del sitio ahora presentan Herbound de forma consistente, manteniendo sin cambios los identificadores de npm package, CLI, repositorio y variables de entorno',
    ...
    new_0_6_10_4: 'La documentacion OpenAPI ahora incluye endpoints de avatar, configuracion de modelos auxiliares, refresh de cache de modelos provider e importacion de sesiones Herbound',
    ...
    new_0_6_9_1: 'La app ahora usa la marca Herbound en el titulo del navegador, la barra lateral, el titulo del sitio, los metadatos del paquete y los enlaces oficiales',
    ...
    new_0_6_7_5: 'La distribución Desktop cubre empaquetado Electron, nombre de app, builds preload, subida de artifacts, inicio Herbound CLI en Windows, iconos Linux y rutas de datos escribibles, macOS sin certificados de firma y subprocesos ocultos al iniciar en Windows',
