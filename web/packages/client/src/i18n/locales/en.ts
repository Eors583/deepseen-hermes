export default {
  ...
  chat: {
    ...
    contextEditHint: 'Common values: 256k (Herbound default), 128k (GPT-4), 32k (GPT-3.5)',
    ...
    emptyState: 'Start a conversation with Herbound',
    ...
    hermesHistory: 'Herbound History',
    historyScopeHint: 'Read-only Herbound history sessions for the current profile, grouped by source.',
    noSessions: 'No sessions',
    searchTitle: 'Search Sessions',
    searchSubtitle: 'Search by title or message content',
    searchScope: 'Search scope: Web UI local session database only. Read-only Herbound history sessions are not included.',
  ...
  plugins: {
    title: 'Plugins',
    refresh: 'Refresh',
    notice: 'Read-only inventory of discoverable Herbound plugin manifests. Discovery metadata is read without loading plugin code. Management actions stay in CLI for v1; changes take effect in new Herbound sessions.',
  ...
  models: {
    ...
    copilotAddDetected: 'A GitHub Copilot OAuth token was detected on this machine. Click Add to enable Copilot in Herbound.',
    ...
    copilotDeleteHintGhCli: 'Copilot will be hidden from Herbound. Your gh CLI login is not affected — `gh auth status` will still show you signed in.',
    copilotDeleteHintAppsJson: 'Copilot will be hidden from Herbound. Your VS Code Copilot extension login is not affected.',
    ...
    aliasHint: 'Display-only alias. Herbound still receives the canonical model ID.',
    ...
    visibilityHint: 'Only affects the Web UI model picker and Models page. Herbound CLI provider/model config is not rewritten; calls still use canonical model IDs.',
  ...
  profiles: {
    ...
    rename: 'Rename',
    delete: 'Delete',
    switchTo: 'Switch Herbound Profile',
    switchConfirm: 'This will run `hermes profile use {name}` and change the active Herbound CLI profile. Continue?',
    switchSuccess: 'Herbound active profile switched to "{name}"',
    switchFailed: 'Failed to switch Herbound profile. Gateway may need manual restart.',
  ...
  codingAgents: {
    ...
    codexDescription: "OpenAI CLI and Herbound openai-codex provider flow for repository tasks.",
    copyCommand: "Copy",
    commandCopied: "Command copied",
    ...
    notes: {
      ...
      codexAuth: "Adds Herbound-managed OpenAI Codex OAuth credentials.",
  ...
  skillsUsage: {
    title: 'Skills Usage',
    subtitle: 'Track skill loads and edits from Herbound sessions',
    refresh: 'Refresh',
  ...
  changelog: {
    ...
    new_0_6_10_7: 'Website copy now consistently presents the product as Herbound while keeping npm package, CLI, repository, and environment variable identifiers unchanged',
    new_0_6_10_8: 'OpenAPI docs now include avatar, auxiliary model config, provider model cache refresh, and Herbound session import endpoints',
    ...
    new_0_6_9_1: 'The app is now branded as Herbound across the browser title, sidebar, website title, package metadata, and official homepage links',
    ...
    new_0_6_7_5: 'Desktop distribution now covers Electron packaging, app naming, preload builds, release artifact uploads, Windows Herbound CLI launch, Linux icons and writable data paths, macOS unsigned release handling, and hidden Windows startup subprocesses',
