export default {
  ...
  chat: {
    ...
    contextEditHint: '一般的な値：256k (Herbound 既定), 128k (GPT-4), 32k (GPT-3.5)',
    contextEditSave: '保存',
    ...
    emptyState: 'Herbound と会話を開始しましょう',
    ...
    openHistory: '履歴を開く',
    hermesHistory: 'Herbound 履歴',
    historyScopeHint: '現在の profile の Herbound 履歴セッションをソース別に読み取り専用で表示します。',
    ...
    searchScope: '検索範囲: Web UI のローカルセッション DB のみ。読み取り専用の Herbound 履歴セッションは含まれません。',
  ...
  plugins: {
    title: 'プラグイン',
    refresh: '更新',
    notice: '検出可能な Herbound プラグイン manifest の読み取り専用インベントリです。検出メタデータはプラグインコードを読み込まずに取得します。v1 の管理操作は CLI のままで、変更は新しい Herbound セッションで有効になります。',
  ...
  models: {
    ...
    copilotAddDetected: 'このマシンで GitHub Copilot OAuth トークンを検出しました。「追加」をクリックして Herbound で Copilot を有効化します。',
    ...
    copilotDeleteHintGhCli: 'Copilot は Herbound 上で非表示になります。gh CLI のログインには影響しません — `gh auth status` は引き続きログイン状態を表示します。',
    copilotDeleteHintAppsJson: 'Copilot は Herbound 上で非表示になります。VS Code Copilot 拡張機能のログインには影響しません。',
    customBadge: 'カスタム',
    previewBadge: 'プレビュー',
    disabledBadge: '利用不可',
    ...
    visibilityHint: 'Web UI のモデル選択とモデルページの表示だけに影響します。Herbound CLI の provider/model 設定は変更されず、実際の呼び出しは元のモデル ID を使います。',
    ...
    aliasManage: '表示名',
    aliasHint: 'Web UI の表示名だけを変更します。Herbound には元のモデル ID が送信されます。',
    aliasEdit: '名前変更',
  ...
  profiles: {
    ...
    rename: '名前変更',
    delete: '削除',
    switchTo: 'Herbound Profile を切り替え',
    switchConfirm: '`hermes profile use {name}` を実行し、Herbound CLI の active profile を切り替えます。続行しますか？',
    switchSuccess: 'Herbound active profile を「{name}」に切り替えました',
    switchFailed: 'Herbound Profile の切り替えに失敗しました。ゲートウェイの手動再起動が必要な場合があります。',
  ...
  codingAgents: {
    ...
    codexDescription: "リポジトリ作業向けの OpenAI CLI と Herbound openai-codex プロバイダーフローです。",
    copyCommand: "コピー",
    commandCopied: "コマンドをコピーしました",
    ...
    notes: {
      ...
      codexAuth: "Herbound 管理の OpenAI Codex OAuth 認証情報を追加します。",
  ...
  skillsUsage: {
    title: 'スキル使用状況',
    subtitle: 'Herbound セッションでのスキル読み込みと編集を追跡します',
    refresh: '更新',
  ...
  // 更新履歴
  changelog: {
    new_0_6_10_1: '公式サイトの文言を Herbound に統一し、npm package、CLI、repository、環境変数の識別子はそのまま保持しました',
    ...
    new_0_6_10_4: 'OpenAPI docs に avatar、auxiliary model config、provider model cache refresh、Herbound session import endpoints を追加しました',
    ...
    new_0_6_9_1: 'アプリのブランドを Herbound に統一し、ブラウザタイトル、サイドバー、Web サイトタイトル、package metadata、公式サイトリンクに反映しました',
    ...
    new_0_6_7_5: 'Desktop 配布は Electron packaging、アプリ名、preload build、release artifact upload、Windows Herbound CLI 起動、Linux icon と書き込み可能 data path、macOS 未署名対応、Windows 起動時の非表示 subprocess に対応しました',
