export default {
  ...
  chat: {
    ...
    contextEditHint: 'Valores comuns: 256k (Herbound padrão), 128k (GPT-4), 32k (GPT-3.5)',
    ...
    emptyState: 'Inicie uma conversa com o Herbound',
    ...
    hermesHistory: 'Histórico Herbound',
    historyScopeHint: 'Sessões do histórico Herbound do perfil atual, somente leitura, agrupadas por origem.',
    ...
    searchScope: 'Escopo da busca: apenas banco local de sessões da Web UI; sessões históricas Herbound somente leitura não são incluídas.',
...
jobTriggered: 'Job acionado',
  ...
  plugins: {
    title: 'Plugins',
    refresh: 'Atualizar',
    notice: 'Inventário somente leitura dos manifests de plugins Herbound detectáveis. Os metadados de descoberta são lidos sem carregar código de plugin. No v1, ações de gerenciamento ficam na CLI; mudanças entram em vigor em novas sessões Herbound.',
  ...
  models: {
    ...
    copilotAddDetected: 'Foi detectado um token OAuth do GitHub Copilot nesta máquina. Clique em Adicionar para ativar o Copilot no Herbound.',
    ...
    copilotDeleteHintGhCli: 'O Copilot ficará oculto no Herbound. Sua sessão no gh CLI não é afetada — `gh auth status` continuará indicando que está conectado.',
    copilotDeleteHintAppsJson: 'O Copilot ficará oculto no Herbound. A extensão Copilot do VS Code continuará conectada.',
    ...
    visibilityHint: 'Afeta apenas o seletor de modelos e a página de modelos da Web UI. A configuração provider/model da Herbound CLI não é alterada; as chamadas ainda usam o ID original do modelo.',
    ...
    aliasHint: 'Altera apenas o nome exibido na Web UI. O Herbound ainda recebe o ID original do modelo.',
    aliasEdit: 'Renomear',
  ...
  profiles: {
    ...
    rename: 'Renomear',
    delete: 'Excluir',
    switchTo: 'Trocar Herbound Profile',
    switchConfirm: 'Isto executara `hermes profile use {name}` e alterara o active profile do Herbound CLI. Continuar?',
    switchSuccess: 'Herbound active profile alterado para "{name}"',
    switchFailed: 'Falha ao trocar Herbound Profile. O gateway pode precisar de reinicio manual.',
  ...
  codingAgents: {
    ...
    codexDescription: "CLI da OpenAI e fluxo de provedor openai-codex do Herbound para tarefas de repositório.",
    copyCommand: "Copiar",
    commandCopied: "Comando copiado",
    ...
    notes: {
      ...
      codexAuth: "Adiciona credenciais OAuth do OpenAI Codex gerenciadas pelo Herbound.",
  ...
  skillsUsage: {
    ...
    subtitle: 'Acompanhe carregamentos e edições de habilidades nas sessões Herbound',
    refresh: 'Atualizar',
  ...
  changelog: {
    new_0_6_10_1: 'Os textos do site agora apresentam Herbound de forma consistente, mantendo identificadores de npm package, CLI, repositorio e variaveis de ambiente inalterados',
    ...
    new_0_6_10_4: 'A documentacao OpenAPI agora inclui endpoints de avatar, configuracao de modelos auxiliares, refresh de cache de modelos provider e importacao de sessoes Herbound',
    ...
    new_0_6_9_1: 'O app agora usa a marca Herbound no titulo do navegador, sidebar, titulo do site, metadados do pacote e links oficiais',
    ...
    new_0_6_7_5: 'A distribuição Desktop cobre empacotamento Electron, nome do app, builds preload, upload de artifacts, início do Herbound CLI no Windows, ícones Linux e caminhos de dados graváveis, macOS sem certificado de assinatura e subprocessos Windows ocultos no início',
