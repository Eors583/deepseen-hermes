export default {
  ...
  chat: {
    ...
    openHistory: '기록 열기',
    hermesHistory: 'Herbound 기록',
    historyScopeHint: '현재 profile의 Herbound 기록 세션을 소스별로 읽기 전용으로 봅니다.',
    noSessions: '세션 없음',
    newChat: '새 채팅',
    ...
    searchScope: '검색 범위: Web UI 로컬 세션 DB만 포함하며 읽기 전용 Herbound 기록 세션은 포함하지 않습니다.',
  ...
  plugins: {
    title: '플러그인',
    refresh: '새로고침',
    notice: '탐색 가능한 Herbound 플러그인 manifest의 읽기 전용 인벤토리입니다. 탐색 메타데이터는 플러그인 코드를 로드하지 않고 읽습니다. v1의 관리 작업은 CLI에 유지되며, 변경 사항은 새 Herbound 세션에서 적용됩니다.',
  ...
  models: {
    ...
    copilotAddDetected: '이 컴퓨터에서 GitHub Copilot OAuth 토큰이 감지되었습니다. 추가를 클릭하여 Herbound에서 Copilot을 활성화하세요.',
    ...
    copilotDeleteHintGhCli: 'Copilot이 Herbound에서 숨겨집니다. gh CLI 로그인에는 영향이 없으며 `gh auth status`는 여전히 로그인 상태를 표시합니다.',
    copilotDeleteHintAppsJson: 'Copilot이 Herbound에서 숨겨집니다. VS Code Copilot 확장 로그인에는 영향이 없습니다.',
    customBadge: '커스텀',
    previewBadge: '프리뷰',
    disabledBadge: '사용 불가',
    ...
    visibilityHint: 'Web UI의 모델 선택기와 모델 페이지 표시에만 영향을 줍니다. Herbound CLI의 provider/model 설정은 변경되지 않으며 실제 호출은 원본 모델 ID를 사용합니다.',
    ...
    aliasManage: '표시 이름',
    aliasHint: 'Web UI 표시 이름만 변경합니다. Herbound에는 여전히 원본 모델 ID가 전달됩니다.',
    aliasEdit: '이름 변경',
  ...
  profiles: {
    ...
    rename: '이름 변경',
    delete: '삭제',
    switchTo: 'Herbound Profile 전환',
    switchConfirm: '`hermes profile use {name}`를 실행하고 Herbound CLI active profile을 변경합니다. 계속하시겠습니까?',
    switchSuccess: 'Herbound active profile이 "{name}"(으)로 전환되었습니다',
    switchFailed: 'Herbound Profile 전환 실패. 게이트웨이를 수동으로 재시작해야 할 수 있습니다.',
  ...
  codingAgents: {
    ...
    codexDescription: "저장소 작업을 위한 OpenAI CLI 및 Herbound openai-codex 제공업체 흐름입니다.",
    copyCommand: "복사",
    ...
    notes: {
      ...
      codexAuth: "Herbound가 관리하는 OpenAI Codex OAuth 자격 증명을 추가합니다.",
  ...
  skillsUsage: {
    title: '스킬 사용량',
    subtitle: 'Herbound 세션의 스킬 로드와 편집을 추적합니다',
    refresh: '새로고침',
  ...
  // 변경 이력
  changelog: {
    new_0_6_10_1: '공식 사이트 문구를 Herbound 로 일관되게 표시하며 npm package, CLI, repository, 환경 변수 식별자는 그대로 유지합니다',
    ...
    new_0_6_10_4: 'OpenAPI docs 에 avatar, auxiliary model config, provider model cache refresh, Herbound session import endpoints 가 추가되었습니다',
    ...
    new_0_6_9_1: '앱 브랜드를 Herbound 로 통일하고 브라우저 제목, 사이드바, 웹사이트 제목, package metadata, 공식 사이트 링크에 반영했습니다',
    ...
    new_0_6_7_5: 'Desktop 배포는 Electron packaging, 앱 이름, preload build, release artifact upload, Windows Herbound CLI 시작, Linux 아이콘과 쓰기 가능한 데이터 경로, macOS 서명 인증서 없음 처리, Windows 시작 시 숨김 subprocess 를 포함합니다',
