export default {
  ...
  chat: {
    ...
    contextEditHint: 'Valeurs courantes : 256k (Herbound par défaut), 128k (GPT-4), 32k (GPT-3.5)',
    ...
    emptyState: 'Demarrer une conversation avec Herbound',
    ...
    hermesHistory: 'Historique Herbound',
    historyScopeHint: 'Sessions d’historique Herbound du profil actuel en lecture seule, regroupées par source.',
    ...
    searchScope: 'Portée de recherche : base locale des sessions Web UI uniquement ; les sessions d’historique Herbound en lecture seule ne sont pas incluses.',
...
jobTriggered: 'Job declenche',
  ...
  plugins: {
    title: 'Plugins',
    refresh: 'Actualiser',
    notice: 'Inventaire en lecture seule des manifests de plugins Herbound détectables. Les métadonnées de découverte sont lues sans charger le code des plugins. En v1, la gestion reste dans le CLI; les changements prennent effet dans les nouvelles sessions Herbound.',
  ...
  models: {
    ...
    copilotAddDetected: 'Un token OAuth GitHub Copilot a été détecté sur cette machine. Cliquez sur Ajouter pour activer Copilot dans Herbound.',
    ...
    copilotDeleteHintGhCli: 'Copilot sera masqué dans Herbound. Votre connexion gh CLI n\'est pas affectée — `gh auth status` indiquera toujours que vous êtes connecté.',
    copilotDeleteHintAppsJson: 'Copilot sera masqué dans Herbound. Votre extension Copilot de VS Code reste connectée.',
    ...
    visibilityHint: 'Affecte uniquement le sélecteur de modèles et la page Modèles de la Web UI. La configuration provider/model de Herbound CLI n’est pas modifiée ; les appels utilisent toujours l’ID de modèle d’origine.',
    ...
    aliasHint: 'Modifie uniquement le nom affiché dans la Web UI. Herbound reçoit toujours l’ID de modèle d’origine.',
    aliasEdit: 'Renommer',
  ...
  profiles: {
    ...
    rename: 'Renommer',
    delete: 'Supprimer',
    switchTo: 'Changer Herbound Profile',
    switchConfirm: 'Cette action execute `hermes profile use {name}` et change le active profile Herbound CLI. Continuer ?',
    switchSuccess: 'Herbound active profile change vers "{name}"',
    switchFailed: 'Echec du changement Herbound Profile. La passerelle peut necessiter un redemarrage manuel.',
  ...
  codingAgents: {
    ...
    codexDescription: "CLI OpenAI et flux fournisseur Herbound openai-codex pour les tâches de dépôt.",
    copyCommand: "Copier",
    commandCopied: "Commande copiée",
    ...
    notes: {
      ...
      codexAuth: "Ajoute les identifiants OAuth OpenAI Codex gérés par Herbound.",
  ...
  skillsUsage: {
    ...
    subtitle: 'Suivre les chargements et modifications de compétences dans les sessions Herbound',
    refresh: 'Actualiser',
  ...
  changelog: {
    new_0_6_10_1: 'Les textes du site presentent maintenant Herbound de facon coherente, tout en conservant les identifiants npm package, CLI, depot et variables d environnement',
    ...
    new_0_6_10_4: 'La documentation OpenAPI inclut maintenant les endpoints avatar, configuration des modeles auxiliaires, refresh du cache des modeles provider et import de session Herbound',
    ...
    new_0_6_9_1: 'L application utilise maintenant la marque Herbound dans le titre du navigateur, la barre laterale, le titre du site, les metadonnees package et les liens officiels',
    ...
    new_0_6_7_5: 'La distribution Desktop couvre packaging Electron, nom d application, builds preload, uploads des artifacts, lancement Herbound CLI sous Windows, icônes Linux et chemins de données inscriptibles, macOS sans certificat de signature et sous-processus Windows masqués au démarrage',
