...
const sessionEventHandlers = new Map<string, {
  ...
  onToolProgress?: (event: RunEvent) => void
...
function globalToolProgressHandler(event: RunEvent): void {
  const sid = event.session_id
  if (!sid) return

  const handlers = sessionEventHandlers.get(sid)
  if (handlers?.onToolProgress) {
    handlers.onToolProgress(event)
  }
}
...
export function connectChatRun(requestedProfile?: string | null): Socket {
  ...
  if (!globalListenersRegistered) {
    ...
    chatRunSocket.on('tool.progress', globalToolProgressHandler)
