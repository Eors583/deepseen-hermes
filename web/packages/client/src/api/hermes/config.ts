...
export interface ToolCredentialsResponse {
  credentials: ToolCredentialInfo[]
}
...
export async function fetchToolCredentials(): Promise<ToolCredentialsResponse> {
  return request<ToolCredentialsResponse>('/api/hermes/config/tool-credentials')
}

export async function saveToolCredentials(values: Record<string, string>): Promise<{ success: boolean }> {
  return request<{ success: boolean }>('/api/hermes/config/tool-credentials', {
    method: 'PUT',
