...
function readDesktopConnectionConfig() {
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object') {
      ...
      const savedMode = parsed.mode === 'remote' ? 'remote' : 'local'
      const profiles = sanitizeConnectionProfiles(parsed.profiles)

      if (!PRODUCTION_CONNECTION_CONFIG || savedMode === 'remote') {
        config = {
          mode: savedMode,
          remote,
