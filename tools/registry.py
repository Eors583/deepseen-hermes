...
class ToolRegistry:
    ...
    @staticmethod
    def _filter_handler_kwargs(handler, kwargs: dict) -> dict:
        """Pass through only kwargs the handler explicitly accepts.

        Many existing tool handlers only accept the positional ``args`` object.
        This lets newer tools opt in to richer runtime context (for example
        ``session_id`` / ``tool_call_id`` / progress callbacks) without
        breaking older handlers that do not declare ``**kwargs``.
        """
        if not kwargs:
            return {}
        try:
    ...
    def dispatch(self, name: str, args: dict, **kwargs) -> str:
        ...
        * All exceptions are caught and returned as ``{"error": "..."}``
        ...
        try:
            handler_kwargs = self._filter_handler_kwargs(entry.handler, kwargs)
            if entry.is_async:
                from model_tools import _run_async
                return _run_async(entry.handler(args, **handler_kwargs))
