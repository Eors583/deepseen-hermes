...
from hermes_constants import get_hermes_home, display_hermes_home, get_skills_dir, get_runtime_skills_dir
import os
import re
...
def _record_enterprise_skill_usage(skill_name: str | None, event_type: str, tool_name: str) -> None:
    runtime_dir = get_runtime_skills_dir()
    if not runtime_dir:
        return
    try:
        from hermes_cli.enterprise_skills.db import transaction
        from hermes_cli.enterprise_skills.service import service as enterprise_service

        with transaction() as conn:
            enterprise_service.record_runtime_skill_usage(
                conn,
        ...
        logger.debug("Failed to record enterprise skill usage", exc_info=True)
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    """
    # Try the active skills dir first, then fall back to external dirs from config.
...
def skills_list(category: str = None, task_id: str = None) -> str:
    ...
    try:
        ...
        if get_runtime_skills_dir():
            _record_enterprise_skill_usage(None, "listed", "skills_list")
...
def skill_view(
    ...
    try:
        ...
        _record_enterprise_skill_usage(str(skill_name), "viewed", "skill_view")
