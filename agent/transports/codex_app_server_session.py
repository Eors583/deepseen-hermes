    ...
    if isinstance(user_input, list):
        ...
        for item in user_input:
            ...
            if item_type in {"text", "input_text"}:
                ...
                path_hint = item.get("path") or item.get("file_path")
                url_hint = None
                image_url = item.get("image_url")
                if isinstance(image_url, dict):
                    url_hint = image_url.get("url")
                elif isinstance(image_url, str):
                    url_hint = image_url
                name_hint = item.get("name") or item.get("file_name")
                media_hint = item.get("media_type") or item.get("mime_type")
                hint = path_hint or url_hint
                if hint:
                    suffix = []
                    if name_hint:
                        suffix.append(f"name={name_hint}")
                    if media_hint:
                        suffix.append(f"type={media_hint}")
                    meta = f" ({', '.join(suffix)})" if suffix else ""
                    parts.append(f"[image attached: {hint}]{meta}")
                else:
                ...
                if path_hint:
                    ...
                    if media_hint:
                        suffix.append(f"type={media_hint}")
                    meta = f" ({', '.join(suffix)})" if suffix else ""
                    parts.append(f"[file attached: {path_hint}]{meta}")
                else:
