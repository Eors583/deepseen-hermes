...
_INLINE_TOOL_CALL_BLOCK_RE = re.compile(
    r"<tool_call>\s*(\{.*?\})\s*</tool_call>",
    re.DOTALL | re.IGNORECASE,
)
_INLINE_TOOL_CALL_CODE_BLOCK_RE = re.compile(
    r"```(?:json)?\s*(\{.*?\})\s*```",
    re.DOTALL | re.IGNORECASE,
)
...
def _coerce_inline_tool_call_candidate(obj: Any, index: int) -> Optional[SimpleNamespace]:
    if not isinstance(obj, dict):
        return None

    candidate = obj.get("tool_call") if isinstance(obj.get("tool_call"), dict) else obj
    ...
    if isinstance(fn, dict):
        ...
        if isinstance(raw_name, str) and raw_name.strip():
            fn_name = raw_name.strip()
            fn_args = candidate.get("arguments", "{}")
...
def recover_inline_tool_calls(agent: Any, assistant_message: Any) -> list[SimpleNamespace]:
    """Recover tool calls that the model emitted inside assistant text.

    Some providers occasionally reply with pseudo tool-call text instead of
    populating the structured ``tool_calls`` field. Recover the common safe
    cases so the normal execution path can still dispatch the tool:
    - ``<tool_call>{...}</tool_call>``
    - fenced JSON blocks containing ``{"tool_call": {...}}``
    - a bare JSON object as the entire message body
    """
    if getattr(assistant_message, "tool_calls", None):
        return list(getattr(assistant_message, "tool_calls") or [])
    ...
    def _consume_match(raw_json: str, start: int, end: int) -> None:
        try:
            parsed = json.loads(raw_json)
        except Exception:
            return
        tool_call = _coerce_inline_tool_call_candidate(parsed, len(extracted) + 1)
        if tool_call is None:
            return
        extracted.append(tool_call)
        consumed_spans.append((start, end))

    for match in _INLINE_TOOL_CALL_BLOCK_RE.finditer(raw_content):
        _consume_match(match.group(1), match.start(), match.end())

    if not extracted:
        for match in _INLINE_TOOL_CALL_CODE_BLOCK_RE.finditer(raw_content):
            _consume_match(match.group(1), match.start(), match.end())

    if not extracted:
        stripped = raw_content.strip()
        if stripped.startswith("{") and stripped.endswith("}"):
            _consume_match(stripped, 0, len(raw_content))
    ...
    for start, end in consumed_spans:
        if cursor < start:
            cleaned_parts.append(raw_content[cursor:start])
        cursor = max(cursor, end)
    if cursor < len(raw_content):
        cleaned_parts.append(raw_content[cursor:])
    cleaned_content = "\n".join(
        part.strip() for part in cleaned_parts if isinstance(part, str) and part.strip()
    ).strip()
    ...
    try:
        logger.warning(
            "Recovered inline tool call(s) from assistant content: provider=%s model=%s names=%s",
            getattr(agent, "provider", ""),
            getattr(agent, "model", ""),
            [tc.function.name for tc in extracted],
        )
    except Exception:
        pass
    return extracted
...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    """
    recover_inline_tool_calls(agent, assistant_message)
